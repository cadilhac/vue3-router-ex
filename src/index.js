...
function getFunctionInformation(name, fn) {
	...
	var args, fnText, argDecl, last, actualFn
...
function routeParamsAreEqual(p1, p2) {
	...
	if (
		objectHasSome(p1, (pv) => pv === '') ||
		objectHasSome(p2, (pv) => pv === '')
	)
		return routeParamsAreEqual(
			objectRemoveProperties(p1, (p) => p === ''),
			objectRemoveProperties(p2, (p) => p === '')
		)
...
function logError(err) {
	console.log(
		'%c' + err.stack,
		'background: #fcdede;padding: 4px; border-radius:2px;'
	)
}
...
const plugin = {
	install(app, options) {
		...
		app.directive('routeTo', vRouteTo)

		// Ensure the router is passed in options
		const router = options.router
		if (!router)
			throw new Error(
				'You must pass the router to vue-router-ex, like so: app.use(vueRouterEx, { router })'
			)

		/**
		  Hack Vue-Router's push/replace functions to allow routing to the current route with only a history state change (Vue-Router will trigger a duplicate route error because the route path stays the same but this hack stores the state so that we can retrieve it later)
		*/
		const originalPush = router.push
		router.push = (route) => {
			if (route.state) router.vue3RouterEx.historyStatePrep = route.state
			return originalPush.call(this, route)
		}

		const originalReplace = router.replace
		router.replaceWithState = (route) => {
			if (route.state) router.vue3RouterEx.historyStatePrep = route.state
			return originalReplace.call(this, route)
		}
			...
			if (Object.keys(resolves).length) {
				...
				resolveNames.forEach((resolveName) => {
					...
					fnInfo.args.forEach((argName) => {
						...
						if (!existingDep) deps.push({ name: argName })
					...
					if (!existingDep) deps.push(fnInfo)
		...
		async function getAllResolvesValues(annotatedResolves) {
			...
			for (let i = 0; i < annotatedResolves.length; i++) {
				...
				if (!resolve.hasOwnProperty('value')) {
					...
					const args = resolve.args.map(
						(arg) => annotatedResolves.find((d) => d.name == arg).value
					)
					resolve.value = await Promise.resolve(
						resolve.fn.apply(resolve.fn, args)
					)
				}
		...
		async function filterResolves(deps, to, from, resolver) {
			...
			const resolve = to.meta.resolveFilter
				? await to.meta.resolveFilter(resolver, { to, from })
				: Object.keys(to.meta.resolve ?? {})
			...
			for (let i = 0; i < to.matched.length - 1; i++) {
				...
				if (resolve) keep = [...keep, ...Object.keys(resolve)]
			}
		...
		async function getResolveValue(name, resolves) {
			...
			const recursive = async (resolves, index) => {
				...
				for (let i = 0; i < dep.args.length; i++) {
					...
					args.push(
						await recursive(
							resolves,
							resolves.findIndex((dep) => dep.name == arg, index + 1)
						)
					)
				}
			...
			return recursive(
				rev,
				rev.findIndex((dep) => dep.name == name)
			)
		}
		...
		async function callBeforeEnterEnd(to, from, failure) {
			...
			for (let i = 0; i < routes.length; i++) {
				...
				if (match.meta.beforeEnterEnd) {
					try {
						const redirectToRoute = await match.meta.beforeEnterEnd(
							to,
							from,
							failure
						)
						if (
							redirectToRoute &&
							(redirectToRoute.name !== to.name ||
								!routeParamsAreEqual(
									redirectToRoute.params,
									to.params /*, compareRouteParams*/
								))
						)
			...
			try {
				...
				for (let i = 0; i < to.matched.length; i++) {
					...
					if (match.meta.beforeEnterBegin) {
						try {
							const redirectToRoute = await match.meta.beforeEnterBegin(
								to,
								from
							)
				...
				try {
					if (to.matched[to.matched.length - 1]?.meta?.redirectTo) {
						...
						const redirectToRoute = await to.matched[
							to.matched.length - 1
						].meta.redirectTo.call(null, resolver, { to, from })
					...
					return (
						(await callBeforeEnterEnd(to, from, { error: 'resolve' })) ?? false
					)
				}
		...
		router.afterEach(async (to, from, failure) => {
			...
			// Failure or not (but especially when there is one when the destination route is
			// the same as the current one), give a chance to get the history state set in router.push
			if (
				router.vue3RouterEx?.historyState &&
				router.vue3RouterEx?.historyStatePrep
			) {
				// Save the history state in a reactive variable that the route component
				// can get through a model. Without a reactive variable, in case of duplicate route,
				// there would be no way for the component to know that the state changed
				router.vue3RouterEx.historyState.value =
					router.vue3RouterEx.historyStatePrep.props ??
					router.vue3RouterEx.historyStatePrep

				router.vue3RouterEx.historyStatePrep = undefined
			}
...
function useRoutesLoader(router, routeDefinitions) {
	...
	const buildRoutesTree = function (routes) {
		...
		routes.forEach((route) => {
			...
			const parent = route.parent
				? routes.find((r) => r.name === route.parent)
				: null

			if (parent) {
				const parentIndex = orderedRoutes.findIndex(
					(r) => r.name === parent.name
				)

				// Add the parent of the current route
				if (parentIndex == -1) {
					if (routeIndex == -1) orderedRoutes.push(parent)
					// at the end of orderedRoutes if its child is not there already
					else orderedRoutes.splice(parent, 0, parent) // Or just before the child
				}

				// Add the route at the end of orderedRoutes if it is not there already
				if (routeIndex == -1) orderedRoutes.push(route)
				...
				if (routeIndex != -1) array_move(orderedRoutes, routeIndex, 0)
				// Otherwise, place it at the front
				else orderedRoutes.unshift(route)
			}
		})
	...
	routes.forEach((route) => {
		...
		if (route.props)
			throw new Error(
				"Don't add your own 'props' property to the route. It will be overwritten. Instead, use resolves in the route definition to create the needed props."
			)
...
export { useRoutesLoader, defineResolves, plugin }
